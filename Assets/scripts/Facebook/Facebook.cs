...
#region FACEBOOK METHODS
    ...
    public static void postear(string to, string message, string name, string description, string picture, string caption, string link)
    {
        ...
        bs.ExternalCall("postear", final, message, name, description, picture, caption, link);
    }
    ...
    {
        string final = "/" + to + "/feed";
        bs.ExternalCall("postearv", final, message, name, description, picture, caption, source, link);
    }
...
#if UNITY_WEBPLAYER
        //_Loader.StartCoroutine(bs.ExternalEval(jsJson.text);
        //_Loader.StartCoroutine(bs.ExternalEval(FBjavascriptfile.text.Replace("%applicationID%", appID));
#endif
    }
    #endregion
#endif
