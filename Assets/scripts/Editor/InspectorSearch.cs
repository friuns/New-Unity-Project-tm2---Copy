...
public class InspectorSearch : EditorWindow
{
    ...
    protected virtual void OnGUI()
    {
        ...
        //if (GUI.Button("Init"))
        //    foreach (Base2 a in FindObjectsOfTypeIncludingAssets(typeof(Base2)))
        //        a.Init();
        OnGUIMono();
        DrawSearch();
    }
    ...
    private static void OnGUIMono()
    {
        ...
        foreach (var a in GameObject.FindGameObjectsWithTag("EditorGUI"))
            a.GetComponent<Base2>().OnEditorGui();

        if (Selection.activeGameObject != null)
        {
            if (Selection.activeGameObject.GetComponent<Base2>() != null)
                lastObj = Selection.activeGameObject.GetComponent<Base2>();
    ...
    private void DrawSearch()
    {
        ...
        if (search.Length > 2 && types.Contains(ao.GetType()) && !(ago != null && ago.camera != null))
        {
    ...
    void MySetValue(Object c, object value, string prName, SerializedPropertyType type)
    {
        ...
        if (AllObjects)
            array = Object.FindObjectsOfType(c.GetType()).Where(a => a.name == c.name);
        else
        {
