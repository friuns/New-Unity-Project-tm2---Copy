...
public class CurvySpline : bs
{
    ...
    public Vector3 Interpolate(float tf) 
    {
        ...
        return (seg) ? seg.Interpolate(localF) : Vector3.zero;
    }
    ...
    public Quaternion GetOrientationFast(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
        return (seg) ? seg.GetOrientationFast(localF) : Quaternion.identity;
    }
    ...
    /// <summary>
    /// Alter TF to reflect a movement over a certain portion of the spline
    /// </summary>
        ...
        if (tf1 < 0) {
            ...
            switch (clamping) {
                ...
                case CurvyClamping.PingPong:
                    ...
                    if (magpart < distance) {
                        direction *= -1;
                        return MoveBy(ref tf, ref direction, distance - magpart, clamping);
                    }
                    break;
            ...
            switch (clamping) {
                case CurvyClamping.Clamp:
                    magpart = (Interpolate(1) - p).magnitude;
                    tf = 1;
                    if (magpart < distance)
    ...
    /// <summary>
    /// Gets the interpolated position by distance from the spline's start
    /// </summary>
    ...
    /// <summary>
    /// Gets the interpolated position by distance from the spline's start. Unlike InterpolateByDistance() this uses a linear approximation
    /// </summary>
    /// <param name="distance">distance in the range of 0..Length</param>
    /// <returns>the interpolated position</returns>
    ...
    /// <summary>
    /// Gets the normalized tangent by distance from the spline's start.
    /// </summary>
    /// <param name="distance">distance in the range of 0..Length</param>
    ...
    /// <summary>
    /// Gets the normalized tangent by distance from the spline's start. Unlike TangentByDistance() this uses a linear approximation
    /// </summary>
    /// <param name="distance">distance in the range of 0..Length</param>
    /// <returns>the tangent/direction</returns>
    ...
    /// </summary>
    /// <param name="local">get bounds in coordinates local to the spline</param>
    ...
    public Vector3[] GetApproximation() { return GetApproximation(false); }
    /// <summary>
    /// Gets an array containing all approximation points
    /// </summary>
    ...
    public float GetNearestPointTF(Vector3 p)
    {
        ...
        // Get sqrDistance to all Approximation points and store nearest index
        float[] pdist2 = new float[ApproximationPointCount];
        CurvySplineSegment seg = mSegments[0];
        int pdistIdx=0;
        int segIdx = 0;
        ...
        while (seg && pdistIdx<pdist2.Length){
            if (seg.Approximation.Length == 0) return -1;
            pdist2[pdistIdx] = (seg.Approximation[segIdx] - p).sqrMagnitude;
            if (pdist2[pdistIdx] < dist) {
                dist = pdist2[pdistIdx];
                foundIndex = segIdx;
                foundSeg = seg;
            }
            pdistIdx++;
            segIdx++;
            if (segIdx == seg.Approximation.Length) {
                seg = NextSegment(seg);
                segIdx = 0;
            }
        }
        ...
        if (!getPreviousApproximationPoint(foundSeg, foundIndex, out pseg[0], out pidx[0])){
            pseg[0] = pseg[1]; pidx[0] = pidx[1];
        }
        if (!getNextApproximationPoint(foundSeg, foundIndex, out pseg[2], out pidx[2])) {
    ...
    /// </summary>
    /// <param name="P">a point on the spline</param>
    ...
    public CurvySplineSegment NextControlPoint(CurvySplineSegment controlpoint)
    {
        ...
        if (i >= mControlPoints.Count)
            return (Closed) ? mControlPoints[0] : null;
        else
    ...
    public CurvySplineSegment PreviousControlPoint(CurvySplineSegment controlpoint)
    {
        ...
        int i = controlpoint.ControlPointIndex - 1;
        if (i < 0)
            return (Closed) ? mControlPoints[mControlPoints.Count - 1] : null;
        else
            return mControlPoints[i];
    }
    ...
    public Transform NextTransform(CurvySplineSegment controlpoint)
    {
        ...
        if (seg)
            ...
            if (AutoEndTangents && Interpolation!=CurvyInterpolation.Linear)
    ...
    public virtual void Refresh(bool refreshLength, bool refreshOrientation, bool skipIfInitialized)
    {
        ...
        if (skipIfInitialized && IsInitialized) return;
    ...
    /// <summary>
    /// Reload control point gameobjects and sort by name
    /// </summary>
    /// <remarks>Call this if you manipulate control point gameobjects outside the Curvy API</remarks>
    public void ReloadControlPoints()
    {
        mControlPoints = new List<CurvySplineSegment>(GetComponentsInChildren<CurvySplineSegment>(false));
    }
    ...
    {
        //return ((float)mSegments.IndexOf(segment) / Count) + (1f / Count) * localF;
    ...
    {
        //MeasureIt.Log("Refresh Tangents", this, "Curvy");
        for (int i = 0; i < Count; i++)
    ...
    {
        //MeasureIt.Log("Refresh Orientation", this, "Curvy");
        if (Count > 0) {
            Vector3 LastUp = (InitialUpVector == CurvyInitialUpDefinition.ControlPoint) ? this[0].Transform.up : minAxis(this[0].GetTangent(0));
    ...
    bool getPreviousApproximationPoint(CurvySplineSegment seg, int idx, out CurvySplineSegment res, out int residx)
    {
        residx = idx - 1;
        res = seg;
        if (residx <0) {
    ...
    bool getNextApproximationPoint(CurvySplineSegment seg, int idx, out CurvySplineSegment res, out int residx)
    {
        residx = idx + 1;
        res = seg;
